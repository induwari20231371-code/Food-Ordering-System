            ...
            <div key={order.id} className="bg-white rounded-2xl shadow-sm overflow-hidden">
              ...
              >
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_COLORS[order.status as keyof typeof STATUS_COLORS] || 'bg-gray-100 text-gray-600'}`}>
                    {order.status}
                  </span>
                  {order.payment && (
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${PAYMENT_COLORS[order.payment.status as keyof typeof PAYMENT_COLORS] || 'bg-gray-100'}`}>
                      💳 {order.payment.status}
                    </span>
                  )}
                </div>
                <div className="flex items-center gap-4">
                  <span className="text-orange-500 font-bold">LKR {parseFloat(String(order.totalAmount)).toFixed(2)}</span>
                    ...
                    <div className="space-y-1">
                      {order.items?.map((item: any) => (
                        <div key={item.id} className="flex justify-between text-sm text-gray-600">
                          ...
                          <span>LKR {parseFloat(String(item.subtotal)).toFixed(2)}</span>
                        </div>
                      ))}
                    </div>
                  </div>
